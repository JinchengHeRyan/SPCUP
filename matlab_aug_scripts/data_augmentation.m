...
wavs_path = './unseen_aug_SNR20';
wavFiles = dir(fullfile(wavs_path,'*.wav'));
for k = 1:length(wavFiles)
    baseFileName = wavFiles(k).name;
    audio_path = fullfile(wavs_path, baseFileName);
    ffmpeg_path = '/usr/local/bin/';  % PUT YOUR FFMPEG PATH HERE!
